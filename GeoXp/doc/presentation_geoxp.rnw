...
\title{Some examples of use of the \pkg{GeoXp} package (version 1.5.0)}
...
in \pkg{sp} package. It contains both spatial coordinates and characteristics of spatial units. \newline
...
extracted from \code{sp.obj} by using the function \code{coordinates}, which can be applied on
to any Spatial Class object (\code{SpatialPointsDataFrame}, \code{SpatialPolygonsDataFrame}, etc). \newline
...
\noindent The first operation consists in creating a Spatial Object. First, we have to
create a \code{SpatialPoints} object by specifying a $2 \times d$ matrix with longitude and latitude:
...
\noindent The second operation consists in creating a \code{SpatialPointsDataFrame} by
...
principles remain the same.
 \begin{itemize}
  ...
  The option is used for changing the variable names included in the \code{sp.obj@data}
  ...
  \item \code{carte:} in the case where \code{sp.obj} is a \code{SpatialPolygonDataFrame}, the user will have the
   opportunity to draw the polygons of Spatial unit by using the \code{Draw Spatial contours} button in the
   Tk window. However, if the \code{sp.obj} is  a \code{SpatialPointsDataFrame}, the user may specify by using
   the \code{carte} option, a matrix with 2 columns for drawing spatial polygonal contours: x and y coordinates of the
...
For this, we first use the function \code{readShapePoly}, included in the \pkg{maptools}
...
\item cell 2: a scatter plot with \code{coordinates(sp.obj)[,2]} on the x-axis and the mean and median of \code{name.var}
calculated for the $m$ rows on the y-axis,
\item cell 3: a scatter plot with the mean and
median of \code{name.var} calculated for the $q$ columns in x-axis and \code{coordinates(sp.obj)[,1]}
in y-axis,
