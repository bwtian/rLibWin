...
Points can be weighted, $w\left(x_j\right)$ is the neighbor's weight.
...
$A$ is the area of the window, $\bar{n}_0$ and $m_0$ are the limit values of $\bar{n}\left(r\right)$ and $m\left(r\right)$ when $r$ gets larger than the window's size. For \emph{M}, it comes:

\begin{equation}
   \frac{{\bar{n}}_0}{m_0}=\frac{1}{n}\sum_i{\frac{W_\mathcal{N}}{W-w\left(x_i\right)}}
\end{equation}
